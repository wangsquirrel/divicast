...
from tyme4py import AbstractTyme, lunar, sixtycycle, solar  # type: ignore
...
def convert_tyme4py_to_entity(obj: AbstractTyme, target: Type[T]) -> T:
    return target.from_chinese_name(obj.get_name())
...
class ZhuInfo(object):
    ...
    def __init__(self, gan: Tiangan, zhi: Dizhi) -> None:
        self.gan = gan
        self.zhi = zhi
        self.sixty_jiazi = SixtyJiazi(gan, zhi)
...
class BirthChart(object):
    ...
    @classmethod
    def create(cls, dt: datetime.datetime | None, bazi: tuple[str] | None = None) -> 'BirthChart':
        if dt is not None:
            bz = (
            ...
            bc = cls((
                bz.get_year().get_heaven_stem().get_name(),
                bz.get_year().get_earth_branch().get_name(),
                bz.get_month().get_heaven_stem().get_name(),
                bz.get_month().get_earth_branch().get_name(),
                bz.get_day().get_heaven_stem().get_name(),
                bz.get_day().get_earth_branch().get_name(),
                bz.get_hour().get_heaven_stem().get_name(),
                bz.get_hour().get_earth_branch().get_name(),
            ))
        elif bazi is not None and len(bazi) == 8:
            bc = cls(bazi)

        else:
            raise ValueError("Either datetime or bazi must be provided")

        bc.assemble()
        return bc

    def assemble(self) -> None:
        self.yearzhu = ZhuInfo(convert_tyme4py_to_entity(self._bazi.get_year().get_heaven_stem(
        ), Tiangan), convert_tyme4py_to_entity(self._bazi.get_year().get_earth_branch(), Dizhi))
        self.monthzhu = ZhuInfo(convert_tyme4py_to_entity(self._bazi.get_month().get_heaven_stem(
        ), Tiangan), convert_tyme4py_to_entity(self._bazi.get_month().get_earth_branch(), Dizhi))
        self.dayzhu = ZhuInfo(convert_tyme4py_to_entity(self._bazi.get_day().get_heaven_stem(
        ), Tiangan), convert_tyme4py_to_entity(self._bazi.get_day().get_earth_branch(), Dizhi))
        self.bihourzhu = ZhuInfo(convert_tyme4py_to_entity(self._bazi.get_hour().get_heaven_stem(
        ), Tiangan), convert_tyme4py_to_entity(self._bazi.get_hour().get_earth_branch(), Dizhi))

        self.kongwang = self.dayzhu.sixty_jiazi.get_kongwang()
        self.chinese_zodiac = self._year_zhi.chinese_zodiac()
        self.sign = ''
...
def draw_chart(birth_chart: BirthChart) -> None:
    ...
    for zhu, name in zip([birth_chart.yearzhu, birth_chart.monthzhu, birth_chart.dayzhu, birth_chart.bihourzhu],
        ...
        print(f"  星运: {zhu.xingyun.chinese_name}")
        print(f"  自坐: {zhu.zizuo.chinese_name}")
        print(
...
draw_chart(BirthChart.create(datetime.datetime(2025, 10, 10, 15, 39)))
