...
from .geju import Geju, analyze_geju
...
class BirthChart(object):
    # 原始信息
    _birth_solar: datetime.datetime  # 出生的真太阳时
    _gender: Gender  # 性别
    ...
    def __init__(
        ...
        self._child_limit = eightchar.ChildLimit(self._solar_time, gender)
    ...
    def analyze(self) -> None:
        ...
        # 五行分析
        self.chart_analysis = self._analyze_wuxing()
    ...
    def _analyze_wuxing(self) -> dict:
        ...
        返回结构：
        {
            ...
            "金": {"count": N, "score": N},
            "水": {"count": N, "score": N},
            "month_zhi_wuxing": "木",  # 月支五行
            "day_master_has_root": True/False,  # 日主是否有根
            "strongest": "木",  # 最强的五行
        }
        """
        # 基础分值
        BASE_HEAVEN = 1.0  # 天干基础分
        BASE_CANGGAN = {
            "MAIN": 0.8,  # 本气
            "MIDDLE": 0.5,  # 中气
            "SECONDARY": 0.3,  # 余气
        }
        MONTH_ZHI_WEIGHT = 1.5  # 月支五行额外权重
        ...
        # 获取月支五行
        month_zhi_wuxing = self.monthzhu.zhi.belongs_to(Wuxing)
        ...
        for zhu in pillars:
            # 天干
            tg = zhu.gan
            tg_wx = tg.belongs_to(Wuxing)
            wuxing_counts[tg_wx.chinese_name] += 1
            wuxing_scores[tg_wx.chinese_name] += BASE_HEAVEN

            # 地支藏干
            for c in zhu.canggan:
                cg_wx = c.gan.belongs_to(Wuxing)
                ctype = c.canggan_type.name
                weight = BASE_CANGGAN.get(ctype, 0.3)
                wuxing_counts[cg_wx.chinese_name] += 1
                wuxing_scores[cg_wx.chinese_name] += weight

        # 月令加权：月支五行加权重
        wuxing_scores[month_zhi_wuxing.chinese_name] += MONTH_ZHI_WEIGHT

        # 判断日主是否有根
        day_master = self.dayzhu.gan
        day_master_wuxing = day_master.belongs_to(Wuxing)
        day_zhi = self.dayzhu.zhi

        # 检查日支藏干是否含有日主五行
        day_master_has_root = False
        for c in day_zhi.canggan():
            if c.gan.belongs_to(Wuxing) == day_master_wuxing:
        ...
        # 构建结果
        result = {
            "木": {"count": wuxing_counts["木"], "score": wuxing_scores["木"]},
            "火": {"count": wuxing_counts["火"], "score": wuxing_scores["火"]},
            "土": {"count": wuxing_counts["土"], "score": wuxing_scores["土"]},
            "金": {"count": wuxing_counts["金"], "score": wuxing_scores["金"]},
            "水": {"count": wuxing_counts["水"], "score": wuxing_scores["水"]},
            "month_zhi_wuxing": month_zhi_wuxing.chinese_name,
            "day_master_wuxing": day_master_wuxing.chinese_name,
            "day_master_has_root": day_master_has_root,
            "strongest": strongest,
        }
    ...
    def _calc_minggua(self) -> str:
        ...
        if method != "bazhai":
            return ""
        lunar_year = _extract_lunar_year(self._solar_time, self._bazi)
        ...
        while num > 9:
            num -= 9
        if num == 5:
            num = 2 if self._gender == Gender.MAN else 8
    ...
    def _calc_minggong(self) -> str:
        ...
        if method != "month_hour_simple":
            return ""
        lunar_month = _extract_lunar_month(self._solar_time, self._bazi)
        ...
        return ming_zhi.chinese_name


def _extract_lunar_year(solar_time: solar.SolarTime, bazi: lunar.EightChar) -> int | None:
    for source in (
        _safe_call(solar_time, "get_lunar_day"),
        _safe_call(_safe_call(solar_time, "get_lunar_hour"), "get_lunar_day"),
        _safe_call(bazi, "get_year"),
    ):
        year_obj = source
        if year_obj is None:
            continue
        year_val = _safe_year_value(year_obj)
        if year_val is not None:
            return year_val
    return None


def _extract_lunar_month(solar_time: solar.SolarTime, bazi: lunar.EightChar) -> int | None:
    for source in (
        _safe_call(solar_time, "get_lunar_day"),
        _safe_call(_safe_call(solar_time, "get_lunar_hour"), "get_lunar_day"),
        _safe_call(bazi, "get_month"),
    ):
        if source is None:
            continue
        month_val = _safe_month_value(source)
...
def _safe_year_value(obj: object) -> int | None:
...
def _safe_month_value(obj: object) -> int | None:
    for method in ("get_month", "get_index", "get_value", "get_month_value"):
        value = _safe_call(obj, method)
        if isinstance(value, int):
            return value
    if isinstance(obj, int):
...
def build_pillar_shensha(bazi: lunar.EightChar, gender: Gender) -> list[list[Daemon]]:
    ...
    gender_num = 1 if gender == Gender.MAN else 0
    ...
    for item in gods:
        ...
        for name in item:
            try:
                daemons.append(Daemon.from_chinese_name(name))
            except Exception:
                continue
